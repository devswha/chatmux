...
type TunnelRow = Readonly<{ peer_id: unknown; ssh_target: unknown; ssh_port: unknown; local_port: unknown }>;
...
function parseRow(row: TunnelRow): FleetSshTunnelRecord {
  ...
  return { peerId: row.peer_id, sshTarget: row.ssh_target, localPort: parsePort(row.local_port, 'local_port'), ...(row.ssh_port === null ? {} : { sshPort: parsePort(row.ssh_port, 'ssh_port') }) };
}
...
export class FleetSshTunnelsRepository {
  ...
  findByPeerId(peerId: string): FleetSshTunnelRecord | undefined {
    const row = this.db.prepare<[string], TunnelRow>('SELECT peer_id, ssh_target, ssh_port, local_port FROM fleet_ssh_tunnels WHERE peer_id = ?').get(peerId);
    return row === undefined ? undefined : parseRow(row);
  }
  findByTarget(sshTarget: string): FleetSshTunnelRecord | undefined {
    const row = this.db.prepare<[string], TunnelRow>('SELECT peer_id, ssh_target, ssh_port, local_port FROM fleet_ssh_tunnels WHERE ssh_target = ?').get(sshTarget);
    return row === undefined ? undefined : parseRow(row);
  }
  list(): readonly FleetSshTunnelRecord[] {
    return this.db.prepare<[], TunnelRow>('SELECT peer_id, ssh_target, ssh_port, local_port FROM fleet_ssh_tunnels ORDER BY peer_id').all().map(parseRow);
  }
  save(record: FleetSshTunnelRecord): void {
    this.db.prepare(`INSERT INTO fleet_ssh_tunnels (peer_id, ssh_target, ssh_port, local_port)
      VALUES (?, ?, ?, ?) ON CONFLICT(peer_id) DO UPDATE SET ssh_target = excluded.ssh_target,
      ssh_port = excluded.ssh_port, local_port = excluded.local_port`).run(record.peerId, record.sshTarget, record.sshPort ?? null, record.localPort);
  }

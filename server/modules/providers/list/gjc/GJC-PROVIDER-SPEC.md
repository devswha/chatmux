# gjc provider — 구현 기록 (ChatMux, 2026-07-16)

ChatMux의 provider `gjc`(Gajae Code) 구현 기록. 초기 read-only 세션 목록/열람 경계는 codex JSONL 스캐너를 참조했고, 현재 live 실행은 전용 worker와 Rust core를 통과한다. 업스트림 PR 금지 — ChatMux에서 개발.

## 세션 스토어 (실측)
- 위치: `$HOME/.gjc/agent/sessions/<cwd-slug>/<ISO-ts>_<uuid>.jsonl`
  - `<cwd-slug>` = 절대 cwd에서 `$HOME` 스트립 후 `/`→`-`. 예 `/workspace/chatmux` → `-workspace-chatmux`. (표시용; **권위 있는 cwd는 JSONL 헤더**.)
...
- 프론트: `src/components/chat/hooks/useChatProviderState.ts`, `ProviderSelectionEmptyState.tsx`, `ProviderLoginModal.tsx`, `public/api-docs.html` PROVIDER_ORDER.
- **live 실행 경로**: `server/gjc-worker-client.ts` → mandatory `chatmux-core` process host → `server/gjc-worker.ts` → GJC SDK/CLI. React·DB·browser replay는 애플리케이션이 계속 소유한다.

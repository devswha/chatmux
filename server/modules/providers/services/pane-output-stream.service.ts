...
import { assertTmuxPaneIdentity, captureTmuxPane, readTmuxPaneIdentity, readTmuxProcessGeneration } from './tmux-pane-actions.service.js';
...
type Entry = { key: PaneSubscriptionKey; lane: Lane; tmux: TmuxPaneIdentity; process: TmuxProcessGeneration; target?: VerifiedTmuxActionTarget; mintedAt: number; hash: string | null; output: string | null; generation: number; subs: Map<WebSocket, Sub>; inFlight: Promise<void> | null };
...
type ParsedSubscription = { lane: Lane; tmux: TmuxPaneIdentity; process: TmuxProcessGeneration };
function parseSubscription(data: Record<string, unknown>): ParsedSubscription {
  if (data.protocolVersion !== 1 || (data.lane !== 'external' && data.lane !== 'live')) throw new Error('invalid pane subscription');
  return { lane: data.lane, tmux: readTmuxPaneIdentity(data.tmux), process: readTmuxProcessGeneration(data.process) };
}
...
export function createPaneOutputStream(options: PaneOutputStreamOptions = {}) {
  ...
  function attach(sub: Sub, entry: Entry): void {
    ...
    emit(sub.ws, { kind: 'pane.attached', subscriptionId: sub.id, key: sub.key, lane: sub.lane, capturedAtMs: now(), outputHash: entry.hash, ...(sub.knownHash === entry.hash ? {} : { output: entry.output }), verifiedUntilMs: entry.mintedAt + PANE_REMINT_MS });
    sub.attached = true;
  }
  function capture(entry: Entry): Promise<void> {
    ...
    entry.inFlight = (async () => {
      ...
      for (const sub of entry.subs.values()) {
        ...
        else if (changed) emit(sub.ws, { kind: 'pane.output', subscriptionId: sub.id, key: sub.key, capturedAtMs: now(), outputHash, output });
      }
  ...
  async function subscribe(ws: WebSocket, data: Record<string, unknown>): Promise<void> {
    const { lane, tmux, process } = parseSubscription(data);
    const key = paneSubscriptionKey(lane, tmux, process);
    const prior = bySocket.get(ws)?.get(key);
    if (prior) {
      const old = subscriptions.get(prior);
      if (old) {
        discardQueuedFrames(ws, old.id);
        emit(ws, { kind: 'pane.invalidated', subscriptionId: old.id, key: old.key, reason: 'superseded' });
        entries.get(old.key)?.subs.delete(ws);
        removeSub(old);
      }
    }
    let entry = entries.get(key);
    if (!entry) { entry = { key, lane, tmux, process, mintedAt: 0, hash: null, output: null, generation: 0, subs: new Map(), inFlight: null }; entries.set(key, entry); }

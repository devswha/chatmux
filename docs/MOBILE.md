...
| 방식 | 장점 | 제한 |
|---|---|---|
| 모바일 브라우저 | 링크만 열면 바로 사용 가능 | 홈 화면 앱과 세션 푸시 알림은 사용할 수 없음 |
| 설치형 PWA | 홈 화면 실행, 전체 화면 UI, 세션별 푸시 알림 지원 | HTTPS 주소에서 기기별로 한 번 설치해야 함 |

대화와 터미널 제어 기능은 두 방식에서 동일합니다. 알림이 필요하지 않다면 브라우저로
먼저 사용해 본 뒤 PWA를 설치해도 됩니다.
...
알림은 HTTPS 주소에서 설치한 PWA에서 설정합니다.

1. 홈 화면의 ChatMux를 실행합니다.
...
- 브라우저 탭이 아니라 홈 화면에 설치한 PWA를 실행했는지 확인합니다.

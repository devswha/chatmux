...
접속 링크 확인, 브라우저 사용, Android/iOS 설치, 알림과 문제 해결은 [모바일 사용 가이드](../mobile_kr.md)를 참고하세요.
...
| 에이전트 | 채팅 뷰 | 시작 시 모두 허용 |
|---|---|---|
| **Claude Code** | 히스토리 인덱싱 후 | 지원 |
| **Codex CLI** | 히스토리 인덱싱 후 | 지원 |
| **Cursor CLI** | 히스토리 인덱싱 후 | 미지원 |
| **OpenCode** | 히스토리 인덱싱 후 | 지원 |
| **Oh My OpenAgent** (`omo`) | 히스토리 인덱싱 후 | 지원 |
| **Oh My Pi** | 히스토리 인덱싱 후 | 지원 |
| **Gajae Code (GJC)** | 기본 지원 | 해당 없음 |

SSH tmux와 로컬 셸도 터미널 전용 연결로 지원합니다.

**모두 허용**은 새로 시작하는 로컬 세션마다 명시적으로 선택하는 옵션입니다. 선택은 저장되지 않으며 기존 세션이나 원격 세션에는 영향을 주지 않습니다. CLI에 따라 해당 프로바이더에서 검증된 자동 승인 또는 전체 권한 모드를 사용하므로, 도구 실행과 셸 명령, 파일 변경이 추가 확인 없이 진행될 수 있습니다. GJC는 자체 기본 허용 정책을 사용하고 별도 시작 옵션이 없으며, Cursor는 검증된 대화형 전체 권한 옵션이 없습니다.

관리자는 `CHATMUX_DISABLE_FULL_ACCESS=1`을 설정해 이 옵션을 숨기고 서버에서도 모두 허용 시작 요청을 거부할 수 있습니다. 값은 앞뒤 공백과 대소문자를 무시하며, 비어 있거나 `0` 또는 `false`인 경우를 제외한 모든 설정값이 기능을 비활성화합니다. 모두 허용으로 시작한 세션은 사이드바에 경고 배지가 계속 표시됩니다.
...
- [프로덕션 설치](../INSTALL.md)
- [모바일 사용](../mobile_kr.md) ([English](../mobile_eng.md))

...
| エージェント | チャットビュー | 起動時のフルアクセス |
|---|---|---|
| **Claude Code** | 履歴のインデックス後 | 対応 |
| **Codex CLI** | 履歴のインデックス後 | 対応 |
| **Cursor CLI** | 履歴のインデックス後 | 非対応 |
| **OpenCode** | 履歴のインデックス後 | 対応 |
| **Oh My OpenAgent** (`omo`) | 履歴のインデックス後 | 対応 |
| **Oh My Pi** | 履歴のインデックス後 | 対応 |
| **Gajae Code (GJC)** | ネイティブ対応 | 該当なし |

SSH tmux とローカルシェルも、ターミナル専用接続として利用できます。

**フルアクセス**は、新しく起動するローカルセッションごとに明示的に選択するオプションです。設定は保存されず、既存またはリモートのセッションには影響しません。CLI に応じて、そのプロバイダーで検証済みの自動承認またはフルアクセスモードを選択するため、ツール、シェルコマンド、ファイル変更が追加確認なしで実行される場合があります。GJC は独自のデフォルト許可ポリシーを使用し、個別の起動スイッチはありません。Cursor には検証済みの対話型フルアクセスフラグがありません。

管理者は `CHATMUX_DISABLE_FULL_ACCESS=1` を設定すると、このオプションを非表示にし、フルアクセスでの起動要求をサーバー側でも拒否できます。値は前後の空白と大文字・小文字を区別せず、空の値、`0`、`false` 以外のすべての値で機能が無効になります。フルアクセスで起動したセッションには、サイドバーで警告バッジが継続して表示されます。

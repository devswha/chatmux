...
확정일: 2026-07-22 (리브랜딩·포지셔닝 갱신, 이전 기준선 2026-07-16). 이 문서는
제품 방향의 기준선이다. 여기에 없는 기능 투자는 v2 이후 몫이며, 스코프를 바꾸려면 이 문서를 먼저 고친다.

## 한 줄 정의

- **ChatMux = 내 tmux 위의 모든 코딩 에이전트를, 브라우저·모바일에서 gjc식
  채팅으로 보고 말 거는 웹터미널.** GJC·Codex·Claude·Cursor·OpenCode 등 tmux에서
  도는 에이전트를 프로바이더-무관(provider-agnostic)하게 하나의 채팅 UI로 관제한다.
- **v1 = tmux가 주인이고 앱은 창문.** 내 tmux에서 이미 돌고 있는 에이전트들을
  브라우저/폰으로 보고, 말 걸 수 있는 관제창.
- **v2 = 앱이 주인이 되는 것.** 웹/데스크톱이 에이전트 작업을 직접
  실행·관리하는 1급 실행기.
...
| V1-2 | **보기** — GJC·Codex는 transcript(.jsonl) 기반 실시간 채팅 뷰(`chatmux-core watch` / codex rollout JSONL, PR#4), claude/cursor/opencode 등은 터미널 attach 뷰(채팅 승격은 provider-agnostic 목표) | GJC·Codex 완성 |
...
| V1-6 | **무로그인** — `CHATMUX_AUTH=none` 기본. implicit owner 매핑, login/register 404. exposure guard가 무인증 비-loopback 바인드를 기동 차단, `CHATMUX_ALLOW_UNAUTH_REMOTE=1`로만 해제 | 완성 (2026-07-16) |

### v1 동작 구조

```
입력:  브라우저 → 앱 서버 → control tower(127.0.0.1:3019) → tmux pane → GJC
출력:  GJC → transcript(.jsonl) → chatmux-core watch → 앱 서버 → WebSocket → 브라우저
```
...
- 웹 구동 채팅 레인: 앱이 CLI를 headless 자식으로 직접 실행 (5개 프로바이더 전부).
  GJC는 `chatmux-core`(Rust process host) → Node worker → `gjc -p` 경로.
...
- **Codex(openai/codex, Apache-2.0)**: 부분 이식 허용(NOTICE/어트리뷰션 준수).
  ...
  v1 진화의 레퍼런스, sandbox/PTY/Git 조각은 4번 `chatmux-core` 확장 재료.
  외부 codex 세션 "보기"는 `~/.codex/sessions` rollout JSONL을
  `chatmux-core watch` 루트로 추가하는 경로를 우선 검토한다.
- 어느 쪽도 확정된 Rust core + GJC worker 레인을 **대체**하는 용도로 쓰지
  않는다 (로드맵 confirmed decision 위반).

## 버저닝 정책 (2026-07-16 확정)

- ChatMux 자체 semver 라인은 `1.0.0`부터 시작. 업스트림(claudecodeui) `1.36.x`
...
| 2026-07-16 | 무로그인 기본 (`CHATMUX_AUTH=none`) + fail-closed exposure guard 구현 |
...
| 2026-07-22 | 제품명 `Gajae App` → **ChatMux** 리브랜딩. 데이터 루트(`~/.chatmux`)·ENV 프리픽스(`CHATMUX_`)·CLI·URL 스킴·rust crate(`chatmux-core`)·repo 슬러그(`devswha/chatmux`) 전부 통일 |
| 2026-07-22 | 포지셔닝 확정: **멀티에이전트 채팅 웹터미널**. codex 채팅 뷰(PR#4) 편입, claude/cursor/opencode 채팅 승격을 provider-agnostic 목표로 명시 |
